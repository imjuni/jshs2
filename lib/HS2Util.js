(function () {
  ...
  function createSASLConnection (host, port, options) {
    var transport = options.transport || thrift.TSaslClientTransport;
  ...
  function patch () {
    ...
    if (!notEmpty(HS2Util.isPatched)) {
      HS2Util.isPatched = true;
    } else {
      return HS2Util.isPatched;
    }
     ...
     *
     */
    thrift.TSaslClientTransport = require('./TSaslClientTransport');

    if (notEmpty(thrift.createSASLConnection)) {
      thrift.oldCreateSASLConnection = thrift.createSASLConnection;
      thrift.createSASLConnection = createSASLConnection;
    } else {
      thrift.createSASLConnection = createSASLConnection;
    }

    TCLIServiceTypes.THandleIdentifier.prototype.$oldRead = TCLIServiceTypes.THandleIdentifier.prototype.read;
    TCLIServiceTypes.THandleIdentifier.prototype.$oldWrite = TCLIServiceTypes.THandleIdentifier.prototype.write;
    TCLIServiceTypes.THandleIdentifier.prototype.read = readEx;
    TCLIServiceTypes.THandleIdentifier.prototype.write = writeEx;

    return HS2Util.isPatched;
  }

  function makeClassMethod () {
    var $scope = arguments['0'];
    var $func = arguments['1'];
    var $args = _.values(arguments).slice(2);

    return function () {
      return $func.apply($scope, $args.concat(_.values(arguments)));
    };
  }

  HS2Util.patch = patch;
  HS2Util.makeClassMethod = makeClassMethod;
