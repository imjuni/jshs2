...
class HS2Util {
  ...
  static getState(serviceType, id) {
    ...
    const stateIndex = id || serviceType.TTypes.TOperationState.UKNOWN_STATE;
  ...
  static getType(serviceType, typeDesc) {
    ...
    return ['string', typeDesc].reduce((type, description) => {
      ...
      if (description.primitiveEntry) {
        result = TTYPE_ID[description.primitiveEntry.type];
      } else if (description.mapEntry) {
        result = TTYPE_ID[description.mapEntry.type];
      } else if (description.unionEntry) {
        result = TTYPE_ID[description.unionEntry.type];
      } else if (description.arrayEntry) {
        result = TTYPE_ID[description.arrayEntry.type];
      } else if (description.structEntry) {
        result = TTYPE_ID[description.structEntry.type];
      } else if (description.userDefinedTypeEntry) {
        result = TTYPE_ID[description.userDefinedTypeEntry.type];
      }
  ...
  static isFinish(cursor, status) {
    const serviceType = cursor.getConfigure().ServiceType;
