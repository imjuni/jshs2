...
//      };
  ...
  Cursor.prototype.cancel = function (callback) {
    ...
    self.connection.client().CancelOperation(req, function (err, res) {
      if (err) {
        callback(new HS2Error.OperationError(err.message));
      } else if (res.status.statusCode === TTypes.TStatusCode.ERROR_STATUS) {
        callback(new HS2Error.OperationError(res.status.errorMessage));
      } else {
        callback(null);
      }
    });
    ...
    if (self.operation) {
      ...
      self.connection.client().GetResultSetMetadata(req, function (err, res) {
        if (err) {
          ...
          callback(new HS2Error.OperationError(err.message || 'error caused from GetResultSetMetadata'));
        } else if (res.status.statusCode === TTypes.TStatusCode.ERROR_STATUS) {
          debug('getSchema: got exception error(' + res.status.errorMessage + ')');

          callback(new HS2Error.OperationError(res.status.errorMessage || 'error caused from GetResultSetMetadata'));
        } else {
          if (res.schema) {
  ...
  Cursor.prototype.fetchBlock = function (callback) {
    ...
    self.connection.client().FetchResults(req, function (err, res) {
      if (err) {
        ...
        try {
          var rows = _.map(res.results.rows, function (row) {
            return _.map(row.colVals, function (colVal) {
              return self.getValue(colVal);
            });
          });
          ...
          return callback(new HS2Error.FetchError(err.message));
        }
      }
    });
  };
