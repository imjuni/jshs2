(function () {
  ...
  function createService (opt) {
    ...
    service.TTypes = require(util.format('../idl/%s/TCLIService_types.js', opt.connOpt.dir));

    service.TTypesValueToName = _.map(_.keys(service.TTypes.TTypeId), function (typeName) {
      return typeName.replace('_TYPE', '').toLowerCase();
    });

    service.TTypesState = _.map(_.keys(service.TTypes.TOperationState), function (state) {
      return state.replace('_STATE', '').toLowerCase();
    });

    function getState (id) {
      id = id || service.TTypes.TOperationState.UKNOWN_STATE;
      return service.TTypesState[parseInt(id, 10)];
    }

    function getType (typeDesc) {
      for (var i = 0, len = typeDesc.types.length; i < len; i++) {
        if (typeDesc.types[i].primitiveEntry) {
          return service.TTypesValueToName[typeDesc.types[i].primitiveEntry.type|0];
    ...
    function makeArray (storedTypes, columns) {
      ...
      for (rowIdx = 0; rowIdx < rowLen; rowIdx++) {
        ...
        for (colIdx = 0; colIdx < colLen; colIdx++) {
          ...
          if (getIsNull(columns[colIdx][colType].nulls, rowIdx)) {
            row.push($opt.nullStr);
          } else {
            row.push(modifyVal(colType, columns[colIdx][colType].values[rowIdx]));
          }
        }
