(function () {
  ...
  function Cursor (client, sessionHandle, opt) {
        ...
        if (err) {
          ...
          if (_(res.results.columns).first()[_(storedTypes).first()].values.length) {
            ...
            if (typeof filter === 'function') {
              currentRows = filter.call($Cursor, storedTypes, res.results.columns, schemas);
            } else {
              currentRows = makeArray.call($Cursor, storedTypes, res.results.columns, schemas);
            }
      ...
      if (_.isEmpty($store.operationHandle)) {
        ...
        $client.FetchResults(req, function (err, res) {
          if (err) {
            ...
            try {
              if (_(res.results.columns).first()[_(storedType).first()].values.length) {
                setImmediate(function () {
                  cb.call($Cursor, null, res.hasMoreRows, makeArray.call($Cursor, makeStoredType(res.results), res.results.columns));
                });
              } else {
                setImmediate(function () {
                  cb.call($Cursor, null, res.hasMoreRows, []);
                });
              }
            } catch (err) {
              setImmediate(function () {
                cb.call($Cursor, new FetchError(err.message));
              });
            }
          }
        });
        ...
        $client.FetchResults(req, function (err, res) {
          if (err) {
            ...
            try {
              if (_(res.results.columns).first()[_(storedType).first()].values.length) {
                setImmediate(function () {
                  cb.call($Cursor, null, res.hasMoreRows, makeJson.call($Cursor, makeStoredType(res.results), res.results.columns, schemas));
                });
              } else {
                setImmediate(function () {
                  cb.call($Cursor, null, res.hasMoreRows, []);
    ...
    $Cursor.getLog = getLog;
    $Cursor.getOperationStatus = getOperationStatus;
    $Cursor.getSchema = getSchema;
    $Cursor.jsonFetch = jsonFetch;
    $Cursor.fetch = fetch;
    $Cursor.fetchBlock = fetchBlock;
    $Cursor.jsonFetchBlock = jsonFetchBlock;
    $Cursor.eventWithFetch = eventWithFetch;
    $Cursor.eventWithJsonFetch = eventWithJsonFetch;
    $Cursor.close = close;
